...
ggplot(dds_counts_plot %>% filter(geneid %in% geneOI & Day %in% c("D6","D7") & Condition %in% c("500","UPSAG","dRA2UPSAG")) %>% 
...
```{r plot-fig-numbers, fig.width=8.5, fig.height=3, include=FALSE, eval=FALSE}
...
top_combined_comparsisons <- top_combined_comparsisons %>%
  ...
  separate(time_space, into= c("faceting","dim1","vs","dim2"), sep = "_")
  # mutate(dim2=factor(dim2, levels=c("D11_","D9_","D7_","p2_","p1_")),
  #        dim1=factor(dim1, levels=c("pM","p1","D5","D7","D9")))

ggplot(top_combined_comparsisons, aes(x=dim1, y=dim2)) +
